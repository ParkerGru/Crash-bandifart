...
class NoteSplashes extends Option
{
	...
	private override function updateDisplay():String
	{
		return "NoteSplashes " + (!FlxG.save.data.notesplash ? "off" : "on");
	}
}

class PS1Intro extends Option
{
	public function new(desc:String)
	{
		super();
	...
	public override function press():Bool
	{
		FlxG.save.data.ps1shit = !FlxG.save.data.ps1shit;
		display = updateDisplay();
		return true;
	}
	...
	private override function updateDisplay():String
	{
		return "PSoneintro " + (!FlxG.save.data.ps1shit ? "off" : "on");
	}
}
