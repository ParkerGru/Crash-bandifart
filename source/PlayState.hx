...
class PlayState extends MusicBeatState
{
	...
	var grpOrangeNoteSplashes:FlxTypedGroup<OrangeNoteSplash>;
	...
	override public function create()
	{
		...
		grpOrangeNoteSplashes = new FlxTypedGroup<OrangeNoteSplash>();
		var sploosh = new OrangeNoteSplash(100, 100, 0);
		sploosh.alpha = 0.1;
		grpOrangeNoteSplashes.add(sploosh);
		...
		switch (songLowercase)
		{
			...
			case 'wump':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/wump/wumpDialogue'));
			case 'crates':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/crates/cratesDialogue'));
			case 'spin':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/spin/spinDialogue'));
		}
		...
		if (!PlayStateChangeables.Optimize)
		{
			switch (stageCheck)
			{
				...
				case 'island':
					defaultCamZoom = 0.9;
						curStage = 'island';
						var bg:FlxSprite = new FlxSprite(-265, -82).loadGraphic(Paths.image('island', 'crash'));
						if(FlxG.save.data.antialiasing)
							{
								bg.antialiasing = true;
							}
						...
						akuaku = new FlxSprite(245, -200);
						akuaku.frames = Paths.getSparrowAtlas('akuaku', 'crash');
						akuaku.animation.addByPrefix('idle', 'akuaku idle', 24, false);
						akuaku.setGraphicSize(Std.int(akuaku.width * 0.5));
						if(FlxG.save.data.antialiasing)
							{
								akuaku.antialiasing = true;
							}
						if (FlxG.save.data.distractions)
						{
							add(akuaku);
		...
		if (SONG.song.toLowerCase() =='wump' || SONG.song.toLowerCase() =='crates' || SONG.song.toLowerCase() =='spin')
		{
			healthBar.createFilledBar(0xFFD96314, 0xFF31B0D1);
		}
		else
		{
		...
		grpOrangeNoteSplashes.cameras = [camHUD];
	...
	private function popUpScore(daNote:Note):Void
	{
		...
		switch (daRating)
		{
			...
			case 'woah':
				...
				if (!daNote.isSustainNote)
					{
						var recycledNote = grpOrangeNoteSplashes.recycle(OrangeNoteSplash);
						recycledNote.setupOrangeNoteSplash(daNote.x, daNote.y, daNote.noteData);
						grpOrangeNoteSplashes.add(recycledNote);
					}
		}
	...
	override function stepHit()
	{
		...
		#end
		fidgetspinner = [30, 170, 174, 394, 398, 509, 782, 786, 814, 818, 832, 846, 850, 864, 871, 878, 910, 914, 942, 946, 960, 974, 976, 992, 999, 1006, 1084, 1110, 1112, 1114, 1116, 1142, 1144, 1146, 1148, 1174, 1176, 1178, 1180, 1206, 1208, 1210, 1212, 1386, 1390, 1418, 1432];
		if (dad.curCharacter == 'crash' && SONG.song.toLowerCase() == 'spin') {

			if (fidgetspinner.contains(curStep))
			{
				FlxG.sound.play(Paths.sound('speen', 'crash'));
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'island':
				if (FlxG.save.data.distractions)
					{
						akuaku.animation.play('idle', true);
					}
		}
